...
CREATE TABLE IF NOT EXISTS monthly_global_summary (
  summary_month       CHAR(7) PRIMARY KEY CHECK (summary_month ~ '^[0-9]{4}-(0[1-9]|1[0-2])$'),
  expected_rent       NUMERIC(12,2) NOT NULL DEFAULT 0,
  collected_rent      NUMERIC(12,2) NOT NULL DEFAULT 0,
  remaining_due       NUMERIC(12,2) NOT NULL DEFAULT 0,
  updated_at          TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE TABLE IF NOT EXISTS monthly_owner_summary (
  summary_month       CHAR(7) NOT NULL CHECK (summary_month ~ '^[0-9]{4}-(0[1-9]|1[0-2])$'),
  owner_id            UUID NOT NULL REFERENCES owners(owner_id) ON DELETE CASCADE,
  expected_rent       NUMERIC(12,2) NOT NULL DEFAULT 0,
  collected_rent      NUMERIC(12,2) NOT NULL DEFAULT 0,
  remaining_due       NUMERIC(12,2) NOT NULL DEFAULT 0,
  updated_at          TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  PRIMARY KEY (summary_month, owner_id)
);
...
CREATE OR REPLACE FUNCTION refresh_monthly_summary_tables()
RETURNS VOID AS $$
BEGIN
  DELETE FROM monthly_owner_summary;
  DELETE FROM monthly_global_summary;

  INSERT INTO monthly_global_summary (
    summary_month,
    expected_rent,
    collected_rent,
    remaining_due,
  ...
  SELECT
    rl.rent_month,
    COALESCE(SUM(rl.rent_amount), 0),
    COALESCE(SUM(rl.amount_paid), 0),
    COALESCE(SUM(rl.remaining_due), 0),
    NOW()
  ...
  GROUP BY rl.rent_month, s.owner_id;
END;
$$ LANGUAGE plpgsql;

SELECT refresh_monthly_summary_tables();
